...
static TRef crec_ct_tv(jit_State *J, CType *d, TRef dp, TRef sp, cTValue *sval)
{
  ...
  if(ctype_isgcptr(d->info) && tref_isgcv(sp)) {
    return sp;
  }
...
static int crec_call(jit_State *J, RecordFFData *rd, GCcdata *cd)
{
  ...
  if (ctype_isfunc(ct->info)) {
	...
	tr = TREF_TRUE;
          ...
          int cid = ctype_cid(ct->info); int tid = 0;

          if(cid == CTID_GCSTR) tid = IRT_STR;
          else if(cid == CTID_GCTAB) tid = IRT_TAB;

          tr = emitconv(tr, tid, t, 0);
