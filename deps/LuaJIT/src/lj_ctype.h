...
// #define CTINFO_GCPTR 0x20030017
// #define ctype_isgcptr(info) (info == CTINFO_GCPTR)

 #define ctype_isgcptr(info) (info == 0x20030017)
 #define ctype_isgcstr(info) (info == 0x20030019)
 #define ctype_isgctab(info) (info == 0x2003001b)
...
#define CTTYDEF(_) \
  ...
  _(GCOBJ,		1,	CT_VOID, CTALIGN(0)) \
  _(GCPTR,	CTSIZE_PTR,	CT_PTR, CTALIGN_PTR|CTID_GCOBJ) \
  _(GCSTROBJ,		2,	CT_VOID, CTALIGN(0)) \
  _(GCSTR,	CTSIZE_PTR,	CT_PTR, CTALIGN_PTR|CTID_GCSTROBJ) \
  _(GCTABOBJ,		4,	CT_VOID, CTALIGN(0)) \
  _(GCTAB,	CTSIZE_PTR,	CT_PTR, CTALIGN_PTR|CTID_GCTABOBJ) \
  CTTYDEFP(_) \
  /* End of type list. */
...
static const char * ctype_getname(CType * ct)
{
  return strdata((GCstr *)gco2str(gcref(ct->name)));
}
